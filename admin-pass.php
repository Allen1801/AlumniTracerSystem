...
include_once 'Includes/session-expire.php';
    ...
    if (isset($_POST['btn_submit-settings'])){
    $N = $_POST['password'];
    
    $sql = "UPDATE accounts SET settpass='$N' WHERE id=$id";

    mysqli_query($link, $sql);
        $sqllogs = "INSERT INTO logs (name, date ,action) 
    VALUES ('$A', CURRENT_DATE ,'Changed their settings password')";
    
    mysqli_query($link, $sqllogs);
	header ("Location: admin-settings.php");
}

if (isset($_POST['btn_submit-login'])){
    $N = $_POST['password'];
    
    $sql = "UPDATE accounts SET password='$N' WHERE id=$id";

    mysqli_query($link, $sql);
        $sqllogs = "INSERT INTO logs (name, date ,action) 
    VALUES ('$A', CURRENT_DATE ,'Changed their login password')";
...
<!DOCTYPE html>
<html>
<title> Settings | Reset Password | PLPATS </title>
...
<head>
    ...
    .change1:hover, .change2:hover{
        ...
        box-shadow: 0 0 10px 5px #A4C19680;
    }
...
<div class="full">
    <div class="container">
        ...
        <form method="POST">
            ...
            <button type="submit" id="Btn" name="btn_submit-settings" class="change1"> Change Settings Password </button>
            <button type="submit" id="Btn" name="btn_submit-login" class="change2"> Change Login Password </button>
        </form>
         
        <a href="admin-settings.php">Go Back to Settings' Page</a>
    </div>
</div>
