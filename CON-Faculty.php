<?php
    ...
    $sqldean = mysqli_query($link, "SELECT * FROM confaculty where id=1");
    $rowdean = mysqli_fetch_array($sqldean);

    $sqlsecretary = mysqli_query($link, "SELECT * FROM confaculty where id=2");
    $rowsecretary = mysqli_fetch_array($sqlsecretary);

    $sqlhead1 = mysqli_query($link, "SELECT * FROM confaculty where id=3");
    $rowhead1 = mysqli_fetch_array($sqlhead1);

    $sqlft1 = mysqli_query($link, "SELECT * FROM confaculty where id=4");
    $rowft1 = mysqli_fetch_array($sqlft1);

    $sqlft2 = mysqli_query($link, "SELECT * FROM confaculty where id=5");
    $rowft2 = mysqli_fetch_array($sqlft2);

    $sqlft3 = mysqli_query($link, "SELECT * FROM confaculty where id=6");
    $rowft3 = mysqli_fetch_array($sqlft3);

    $sqlft4 = mysqli_query($link, "SELECT * FROM confaculty where id=7");
    $rowft4 = mysqli_fetch_array($sqlft4);

    $sqlft5 = mysqli_query($link, "SELECT * FROM confaculty where id=8");
    $rowft5 = mysqli_fetch_array($sqlft5);

    $sqlft6 = mysqli_query($link, "SELECT * FROM confaculty where id=9");
    $rowft6 = mysqli_fetch_array($sqlft6);

    $sqlft7 = mysqli_query($link, "SELECT * FROM confaculty where id=10");
    $rowft7 = mysqli_fetch_array($sqlft7);

    $sqlft8 = mysqli_query($link, "SELECT * FROM confaculty where id=11");
    $rowft8 = mysqli_fetch_array($sqlft8);

    $sqlft9 = mysqli_query($link, "SELECT * FROM confaculty where id=12");
...
<main> 
    ...
    <div class="affairs containers">
      <h2 class="affairs-title">College of Nursing  </h2>
      <p class="affairs-subtitle">A.Y. 2022-2023  </p>
      <p class="affairs-subtitle" style="margin-top: -20px;"> <i> con_@plpasig.edu.ph </i> </p>
